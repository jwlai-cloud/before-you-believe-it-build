...
- **Live GPT-5.6 is the engine.** A parent enters any general topic and age band, and GPT-5.6 generates the material **live, in real time** — a fresh claim, a hidden assumption, three balanced clues, a parent prompt, and an open uncertainty. **Bounded material to question, never a verdict to obey.** Nothing is pre-scripted: every topic produces a genuinely new challenge, and the demo shows the real request running on screen.
